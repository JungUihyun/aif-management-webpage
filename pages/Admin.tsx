...
const Admin: React.FC = () => {
  ...
  const handleRoleChange = (userId: string, newRole: UserRole) => {
    // In real app: await api.updateUserRole(userId, newRole);
      ...
      <div className="bg-white p-6 rounded-xl shadow-sm border border-gray-100">
          <div className="flex justify-between items-center mb-4">
              <h2 className="font-bold text-gray-800 flex items-center">
                  <div className="w-6 h-6 bg-yellow-300 rounded mr-2 flex items-center justify-center text-xs font-bold text-black">K</div>
          ...
          <p className="text-sm text-gray-500">
              최근 1시간 동안의 입금 내역을 불러와 회비 납부 상태를 자동으로 업데이트합니다.
          </p>
      </div>
              ...
              <table className="min-w-full divide-y divide-gray-200">
                  <thead className="bg-gray-50">
                      <tr>
                          ...
                          <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase">현재 권한</th>
                          <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase">권한 변경</th>
                      </tr>
                  </thead>
                          ...
                          <tr key={user.id}>
                              ...
                              <td className="px-6 py-4 whitespace-nowrap">
                                  <span className={`px-2 inline-flex text-xs leading-5 font-semibold rounded-full 
                                    ${user.role === UserRole.EXECUTIVE ? 'bg-purple-100 text-purple-800' : 
