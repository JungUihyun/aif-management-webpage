...
const Schedule: React.FC = () => {
  ...
  const canCreateMatch = user?.role === UserRole.EXECUTIVE || user?.role === UserRole.MANAGER;
        ...
        <div className="flex items-center space-x-2 w-full sm:w-auto">
            ...
            <div className="flex bg-gray-100 p-1 rounded-lg flex-shrink-0">
                <button 
                    ...
                    className={`px-3 py-1.5 rounded-md text-sm font-medium transition-all ${view === 'list' ? 'bg-white shadow-sm text-primary' : 'text-gray-500'}`}
                >
                    리스트
                </button>
                <button 
                    onClick={() => setView('month')}
                    className={`px-3 py-1.5 rounded-md text-sm font-medium transition-all ${view === 'month' ? 'bg-white shadow-sm text-primary' : 'text-gray-500'}`}
                >
                    달력
                </button>
            </div>
          ...
          <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4">
              <div className="bg-white w-full max-w-md rounded-2xl shadow-xl overflow-hidden animate-in fade-in zoom-in duration-200">
                  ...
                  <form onSubmit={handleCreateMatch} className="p-6 space-y-4">
                      <div>
                          <label className="block text-sm font-bold text-gray-700 mb-1">상대팀 이름</label>
                          <input 
                              type="text" 
                              name="opponent"
                      ...
                      <div className="grid grid-cols-2 gap-4">
                          <div>
                              <label className="block text-sm font-bold text-gray-700 mb-1">경기 날짜</label>
                              <input 
                                  type="date" 
                                  name="date"
                                  value={newMatch.date}
                          ...
                          <div>
                              <label className="block text-sm font-bold text-gray-700 mb-1">경기 시간</label>
                              <input 
                                  type="time" 
                                  name="time"
                                  value={newMatch.time}
                      ...
                      <div>
                          <label className="block text-sm font-bold text-gray-700 mb-1">경기 장소</label>
                          <input 
                              type="text" 
                              name="location"
